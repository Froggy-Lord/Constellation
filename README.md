...
hypixel's chest-backed Craft Item screen can be rearranged into a proper 3x3 SkyBlock crafting interface with server-owned result and quick-craft slots, More Crafts navigation, filler hiding and the distinct Mirrorverse layout. every click still targets the original Hypixel slot. ender chest and backpack pages feed a profile-safe unified storage browser with cross-page search, native item tooltips, custom names/order and deliberate server-owned page navigation.

bazaar, auction house and museum search signs can open a proper searchable overlay backed by live market membership. it keeps separate history for each market, shows exact item suggestions, supports current pet and dungeon-star filters, and sends only the final user-confirmed search. `/ahs` and `/bzs` open the same interface directly.
...
- `/cn config [constellation]` — opens the config screen, optionally at one module
