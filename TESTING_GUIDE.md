...
- [ ] Focus each Inventory Button text field and resize the window. Expected: its value and keyboard focus survive; neither reset action runs until its confirmation is clicked a second time.
...
- [ ] Open Composter, Anita, Visitor's Logbook, Pesthunter's Wares and Configure Plots with their side panels enabled. Expected: each panel appears entirely beside the container, never over slot zero or another server slot.
- [ ] Repeat those five Garden panels at narrow and short window sizes with their maximum row settings. Expected: they choose the larger free gutter, fit their text, cap lower rows before the screen edge and leave vanilla item tooltips authoritative.
- [ ] Open every dungeon reward chest with Chest Profit in wide, standard and narrow windows. Expected: the panel chooses the larger free gutter, never covers reward slots, and always retains fitted cost and profit lines.
- [ ] Disable compact Chest Profit and test a chest with many rewards and unknown prices in a short window. Expected: only the number of loot rows that fit are shown; unknown-price state remains visible and the panel never crosses the bottom edge.
- [ ] Enable Reforge Helper's filter editor and open both Hex and basic Reforge at wide and standard widths. Expected: Wanted and Excluded fields choose the larger free gutter and never overlap or consume clicks intended for server slots.
- [ ] Narrow the window until neither side has 90 free pixels. Expected: both fields are withheld together instead of becoming clipped or covering the menu; saved filters and command configuration remain active.
- [ ] Hover Party Finder listings containing four long player names and loaded API statistics near every screen edge. Expected: the complete detail uses the native tooltip layer, wraps/repositions inside the window and remains above decorative slot marks.
- [ ] Open Party Finder in a narrow window with every class state represented. Expected: the joinable, dupe and blocked summary fits above the container without leaving the screen.
- [ ] Set Spirit Leap scale to 200%, then open it at wide, standard and narrow sizes. Expected: wide windows honor 200%; smaller windows reduce only the effective render scale so all four cards and the title remain visible and clickable, while the saved setting stays 200%.
- [ ] Set Inventory Buttons to size 32, gap 12 and both rows enabled, then narrow the window. Expected: the effective gap reduces first and button size only when necessary; all seven buttons remain inside the viewport and the saved 32/12 preferences return at wider sizes.
- [ ] Hover and click every responsive Inventory Button at both row edges. Expected: visual movement, tooltip activation and click ownership use the same rectangle, with no invisible or displaced hit target.
