...
public final class ItemValueCalculator {
    ...
    public static Result estimate(ItemStack stack) {
        return estimate(stack, true);
    }

    /** Same estimate without launching missing-price requests; useful for bounded bulk calculations. */
    public static Result estimateCached(ItemStack stack) {
        return estimate(stack, false);
    }

    private static Result estimate(ItemStack stack, boolean warm) {
        ...
        if (base <= 0 && warm && !marketId.isBlank()) PriceProvider.warm(marketId);
        boolean[] complete = {base > 0};
        List<ComponentValue> parts = new ArrayList<>();

        add(parts, complete, warm, "HOT_POTATO_BOOK", Math.min(10, positive(extra, "hot_potato_count")));
        add(parts, complete, warm, "FUMING_POTATO_BOOK", Math.max(0, positive(extra, "hot_potato_count") - 10));
        add(parts, complete, warm, "RECOMBOBULATOR_3000", positive(extra, "rarity_upgrades"));
        add(parts, complete, warm, "FARMING_FOR_DUMMIES", positive(extra, "farming_for_dummies_count"));
        add(parts, complete, warm, "OVERCLOCKER_3000", positive(extra, "levelable_overclocks"));
        add(parts, complete, warm, "POLARVOID_BOOK", positive(extra, "polarvoid"));
        add(parts, complete, warm, "BOOKWORM_BOOK", positive(extra, "bookworm_books"));
        add(parts, complete, warm, "TRANSMISSION_TUNER", positive(extra, "tuned_transmission"));
        add(parts, complete, warm, "MANA_DISINTEGRATOR", positive(extra, "mana_disintegrator_count"));
        add(parts, complete, warm, "JALAPENO_BOOK", positive(extra, "jalapeno_count"));
        add(parts, complete, warm, "WOOD_SINGULARITY", positive(extra, "wood_singularity_count"));
        add(parts, complete, warm, "THE_ART_OF_WAR", positive(extra, "art_of_war_count"));
        add(parts, complete, warm, "WET_BOOK", positive(extra, "wet_book_count"));
        add(parts, complete, warm, "POCKET_SACK_IN_A_SACK", positive(extra, "sack_pss"));
        add(parts, complete, warm, "DIVAN_POWDER_COATING", present(extra, "divan_powder_coating"));
        add(parts, complete, warm, "BOOK_OF_STATS", present(extra, "stats_book"));
        add(parts, complete, warm, "THE_ART_OF_PEACE", present(extra, "artOfPeaceApplied"));
        add(parts, complete, warm, "ETHERWARP_MERGER", present(extra, "ethermerge"));
        add(parts, complete, warm, "SIL_EX", Math.max(positive(extra, "silex"), positive(extra, "silex_count")));
        add(parts, complete, warm, "MITHRIL_INFUSION", positiveByte(extra, "mithril_infusion"));
        add(parts, complete, warm, "FREE_WILL", positiveByte(extra, "free_will"));

        int upgrades = Math.max(positive(extra, "upgrade_level"), positive(extra, "dungeon_item_level"));
        for (int i = 0; i < Math.min(5, Math.max(0, upgrades - 5)); i++) add(parts, complete, warm, MASTER_STARS[i], 1);
        addStringComponent(parts, complete, warm, extra, "skin");
        addStringComponent(parts, complete, warm, extra, "dye_item");
        addStringComponent(parts, complete, warm, extra, "power_ability_scroll");
        addStringComponent(parts, complete, warm, extra, "drill_part_upgrade_module");
        addStringComponent(parts, complete, warm, extra, "drill_part_engine");
        addStringComponent(parts, complete, warm, extra, "drill_part_fuel_tank");
        ...
        if (!rawId.equals("ENCHANTED_BOOK")) {
            ...
            for (String key : enchants.keySet()) {
                ...
                if (level > 0) add(parts, complete, warm, "ENCHANTMENT_" + key.toUpperCase(Locale.ROOT) + "_" + level, 1);
            }
        }
        if (!rawId.equals("RUNE") && !rawId.equals("UNIQUE_RUNE")) {
            ...
            for (String key : runes.keySet()) {
                ...
                if (level > 0) add(parts, complete, warm, key.toUpperCase(Locale.ROOT) + "_" + level + "_RUNE", 1);
            }
        }
        ...
        for (String key : gems.keySet()) {
            ...
            if (quality.matches("ROUGH|FLAWED|FINE|FLAWLESS|PERFECT")) {
                ...
                add(parts, complete, warm, quality + "_" + type + "_GEM", 1);
        ...
        for (Tag tag : extra.getListOrEmpty("ability_scroll")) {
            ...
            if (scroll.equals("ULTIMATE_WITHER_SCROLL")) {
                add(parts, complete, warm, "IMPLOSION_SCROLL", 1);
                add(parts, complete, warm, "WITHER_SHIELD_SCROLL", 1);
                add(parts, complete, warm, "SHADOW_WARP_SCROLL", 1);
            } else if (!scroll.isBlank()) add(parts, complete, warm, scroll, 1);
        }
        double total = base;
    ...
    private static void addStringComponent(List<ComponentValue> parts, boolean[] complete, boolean warm, CompoundTag extra, String key) {
        String id = extra.getStringOr(key, "").trim();
        if (!id.isBlank()) add(parts, complete, warm, id.toUpperCase(Locale.ROOT), 1);
    }

    private static void add(List<ComponentValue> parts, boolean[] complete, boolean warm, String id, int count) {
        if (count <= 0 || id.isBlank()) return;
        double price = PriceProvider.purchaseValue(id);
        if (price <= 0) {
            if (warm) PriceProvider.warm(id);
            complete[0] = false;
            return;
        }
