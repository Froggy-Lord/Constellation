...
public final class AurigaHoppityWaypoints {
    ...
    public static void init(AurigaConfig config){
        cfg=config;normalize();load();AurigaHoppityNavigation.init(config);
    ...
    private static Location pathTarget(Minecraft mc){if(!active()||!cfg.hoppityEggPath)return null;if(guess!=null)return guess;if(cfg.hoppityEggPathOnlyGuess)return null;return locations().stream().filter(value->!collected(value)).min(Comparator.comparingDouble(value->value.pos.distanceToSqr(mc.player.position()))).orElse(null);}
    ...
    public static void registerCommands(CommandDispatcher<FabricClientCommandSource> dispatcher){
        dispatcher.register(LiteralArgumentBuilder.<FabricClientCommandSource>literal("hoppitywaypoints")
            ...
            .then(LiteralArgumentBuilder.<FabricClientCommandSource>literal("pathwidth")
                .then(RequiredArgumentBuilder.<FabricClientCommandSource,Integer>argument("pixels",IntegerArgumentType.integer(1,15)).executes(context->number("pathwidth",IntegerArgumentType.getInteger(context,"pixels")))))
            .then(LiteralArgumentBuilder.<FabricClientCommandSource>literal("pathlookahead")
                .then(RequiredArgumentBuilder.<FabricClientCommandSource,Integer>argument("nodes",IntegerArgumentType.integer(2,5000)).executes(context->number("pathlookahead",IntegerArgumentType.getInteger(context,"nodes")))))
            .then(LiteralArgumentBuilder.<FabricClientCommandSource>literal("pathmovement")
                .then(RequiredArgumentBuilder.<FabricClientCommandSource,Integer>argument("blocks",IntegerArgumentType.integer(1,16)).executes(context->number("pathmovement",IntegerArgumentType.getInteger(context,"blocks")))))
            .then(LiteralArgumentBuilder.<FabricClientCommandSource>literal("pathinterval")
                .then(RequiredArgumentBuilder.<FabricClientCommandSource,Integer>argument("ticks",IntegerArgumentType.integer(2,100)).executes(context->number("pathinterval",IntegerArgumentType.getInteger(context,"ticks")))))
            .then(LiteralArgumentBuilder.<FabricClientCommandSource>literal("pathlimit")
                .then(RequiredArgumentBuilder.<FabricClientCommandSource,Integer>argument("nodes",IntegerArgumentType.integer(100,20000)).executes(context->number("pathlimit",IntegerArgumentType.getInteger(context,"nodes")))))
    ...
    private static int status(){long collected=locations().stream().filter(AurigaHoppityWaypoints::collected).count();String path=!AurigaHoppityNavigation.supported()?"unsupported":AurigaHoppityNavigation.routeNodes()==0?"idle":AurigaHoppityNavigation.routeNodes()+" nodes, "+AurigaHoppityNavigation.routeDistance()+"m";local(locations().size()+" locations on "+lastArea+", "+collected+" collected, locator "+(guess==null?"idle":"solved")+", path "+path+".");return 1;}
    ...
    private static void normalize(){if(cfg.hoppityCollectedEggLocations==null)cfg.hoppityCollectedEggLocations=new LinkedHashMap<>();cfg.hoppityEggWaypointRange=Math.clamp(cfg.hoppityEggWaypointRange,16,5000);cfg.hoppityEggLocatorParticleGapTenths=Math.clamp(cfg.hoppityEggLocatorParticleGapTenths,5,50);cfg.hoppityEggLocatorTimeoutSeconds=Math.clamp(cfg.hoppityEggLocatorTimeoutSeconds,1,10);cfg.hoppityEggLocationClaimRadius=Math.clamp(cfg.hoppityEggLocationClaimRadius,3,30);cfg.hoppityEggPathWidth=Math.clamp(cfg.hoppityEggPathWidth,1,15);cfg.hoppityEggPathLookAheadNodes=Math.clamp(cfg.hoppityEggPathLookAheadNodes,2,5000);cfg.hoppityEggPathRecalculateBlocks=Math.clamp(cfg.hoppityEggPathRecalculateBlocks,1,16);cfg.hoppityEggPathRecalculateTicks=Math.clamp(cfg.hoppityEggPathRecalculateTicks,2,100);cfg.hoppityEggPathMaxVisitedNodes=Math.clamp(cfg.hoppityEggPathMaxVisitedNodes,100,20000);}
