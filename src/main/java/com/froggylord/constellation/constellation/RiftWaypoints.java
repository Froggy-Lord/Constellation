...
/**
 * Rift Mirrorverse waypoints — verified positions from Skyblocker's
 * mirrorverse_waypoints.json data. Renders beams + markers through walls to
 * guide you through the Mirrorverse parkour.
 */
public final class RiftWaypoints {

    private RiftWaypoints() {}

    // Mirrorverse sections with their waypoints (from Skyblocker data)
    ...
    // Enigma soul waypoints from official Hypixel Wiki data (Skyblocker source)
    ...
    public static void draw(WorldRenderer.Ctx ctx) {
        ...
        // Mirrorverse waypoints — show all sections
        ...
        if (showSouls) {
            for (int[] p : ENIGMA_SOULS) {
                ...
                if (dist > 2500) continue; // only nearby souls
