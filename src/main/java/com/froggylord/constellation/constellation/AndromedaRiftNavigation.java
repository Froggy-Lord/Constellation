...
// ported from SkyHanni (LGPL-3.0-or-later): data/IslandGraphs.kt, utils/GraphUtils.kt, data/model/graph/DijkstraTree.kt
...
public final class AndromedaRiftNavigation {
    ...
    private static AndromedaConfig cfg;private static boolean loaded;private static int target=-1,ticks,lastArrivalTarget=-1;private static Node nearest,targetNode;private static Route route;private static Vec3 lastPlayer;
    ...
    private static void tick(){
        if(!active()){clearTransient();return;}Minecraft mc=Minecraft.getInstance();if(mc.player==null)return;
        ...
        Vec3 player=mc.player.position();Node now=nearest(player);boolean moved=lastPlayer==null||lastPlayer.distanceToSqr(player)>9;
    ...
    private static void rebuild(){
        ...
        targetNode=nearest(Vec3.atCenterOf(goal));Route found=shortest(nearest,targetNode);if(found==null){route=null;return;}
    ...
    private static Node nearest(Vec3 position){Node best=null;double distance=Double.POSITIVE_INFINITY;for(Node node:NODES.values()){double current=node.pos.distanceToSqr(position);if(current<distance){distance=current;best=node;}}return best;}
    private static Route shortest(Node start,Node end){
        ...
        while(!queue.isEmpty()){QueueNode current=queue.poll();if(current.distance>distance.getOrDefault(current.node,Double.POSITIVE_INFINITY))continue;if(current.node==end)break;for(Edge edge:current.node.edges){double next=current.distance+edge.weight;if(next>=distance.getOrDefault(edge.to,Double.POSITIVE_INFINITY))continue;distance.put(edge.to,next);previous.put(edge.to,current.node);queue.add(new QueueNode(edge.to,next));}}
    ...
    public static void draw(WorldRenderer.Ctx ctx){
        if(!active()||target<0||route==null||route.nodes.isEmpty())return;Minecraft mc=Minecraft.getInstance();if(mc.player==null)return;
        ...
        if(cfg.riftPathfinderTargetLabel){String label="Enigma Soul "+(target+1);if(cfg.riftPathfinderDistance)label+=" "+Math.round(route.distance)+"m";ctx.label(center.add(0,1.3,0),label,color,cfg.riftPathfinderThroughWalls);}
    }
    private static void checkArrival(Vec3 player){BlockPos pos=AndromedaRiftCore.soulPosition(target);if(pos==null||pos.distToCenterSqr(player)>Math.pow(Math.clamp(cfg.riftPathfinderArrivalRange,2,12),2))return;if(cfg.riftPathfinderArrivalChat&&lastArrivalTarget!=target)local("Reached Enigma Soul "+(target+1)+".");lastArrivalTarget=target;if(cfg.riftPathfinderArrivalClear&&!cfg.riftPathfinderAutoNearest)clear();}
    ...
    private static void select(int index){if(index<0||index>=AndromedaRiftCore.soulCount()){local("Soul number must be 1 to "+AndromedaRiftCore.soulCount()+".");return;}target=index;lastArrivalTarget=-1;resetRoute();local("Routing to Enigma Soul "+(index+1)+".");}
    ...
    private static void clearTransient(){resetRoute();}
