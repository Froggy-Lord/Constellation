...
public final class SpeedPresetScreen extends Screen {
    ...
    @Override public boolean mouseClicked(MouseButtonEvent event,boolean dbl){
        ...
        int y=52-scroll;for(var entry:rows()){if(my>=52&&my<height-53&&inside(mx,my,12,y,width-24,17)){selected=entry.getKey();name.setValue(entry.getKey());speed.setValue(Integer.toString(entry.getValue()));return true;}y+=20;}return super.mouseClicked(event,dbl);
    }
