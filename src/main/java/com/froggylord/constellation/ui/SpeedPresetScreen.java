...
public final class SpeedPresetScreen extends Screen {
    ...
    @Override public void extractBackground(GuiGraphicsExtractor graphics,int mx,int my,float delta){
        ...
        int actions=actionX(),actionY=actionY();button(graphics,actions,actionY,52,"Save",mx,my);button(graphics,actions+56,actionY,52,confirmDelete?"Confirm":"Delete",mx,my);button(graphics,actions+112,actionY,48,"Use",mx,my);button(graphics,actions+164,actionY,54,confirmReset?"Confirm":"Reset",mx,my);button(graphics,width-68,height-23,54,"Done",mx,my);if(!message.isBlank())graphics.text(font,ConstellationUi.fit(font,message,Math.max(20,width-96)),14,height-22,messageError?0xFFFF7777:0xFF77FFAA,false);
    }
    @Override public boolean mouseClicked(MouseButtonEvent event,boolean dbl){
        int mx=(int)event.x(),my=(int)event.y(),actions=actionX(),actionY=actionY();if(inside(mx,my,actions,actionY,52,18)){confirmDelete=confirmReset=false;saveEntry();return true;}if(inside(mx,my,actions+56,actionY,52,18)){if(selected.isBlank()){message="Select a preset to delete";messageError=true;confirmDelete=confirmReset=false;}else if(confirmDelete){PhoenixSpeedPresets.remove(selected);selected="";name.setValue("");speed.setValue("");message="Preset deleted";messageError=false;confirmDelete=false;}else{confirmDelete=true;confirmReset=false;}return true;}if(inside(mx,my,actions+112,actionY,48,18)){confirmDelete=confirmReset=false;String value=selected.isBlank()?name.getValue():selected;if(!value.isBlank())PhoenixSpeedPresets.use(value);return true;}if(inside(mx,my,actions+164,actionY,54,18)){if(confirmReset){PhoenixSpeedPresets.reset();selected="";message="Defaults restored";messageError=false;confirmReset=false;}else{confirmReset=true;confirmDelete=false;}return true;}if(inside(mx,my,width-68,height-23,54,18)){onClose();return true;}
        int y=52-scroll;for(var entry:rows()){if(my>=52&&my<listBottom()&&inside(mx,my,12,y,width-24,17)){confirmDelete=confirmReset=false;selected=entry.getKey();name.setValue(entry.getKey());speed.setValue(Integer.toString(entry.getValue()));return true;}y+=20;}confirmDelete=confirmReset=false;return super.mouseClicked(event,dbl);
    }
    ...
    @Override public boolean mouseScrolled(double mx,double my,double sx,double sy){int max=Math.max(0,rows().size()*20-(listBottom()-52));scroll=Math.clamp(scroll-(int)(sy*24),0,max);return true;}
    ...
    private boolean narrow(){return width<440;}
    private int actionX(){return narrow()?Math.max(8,(width-218)/2):Math.max(194,width-232);}
    private int actionY(){return narrow()?height-70:height-47;}
    private int listBottom(){return height-(narrow()?76:53);}
