...
public final class PetDisplayHudWidget implements HudElement {
    ...
    @Override public int width(){Font f=Minecraft.getInstance().font;int w=0;for(String line:lines())w=Math.max(w,f.width(line));return w+(PhoenixPetDisplay.config().petDisplayIcon&&!PhoenixPetDisplay.icon().isEmpty()?20:0);}@Override public int height(){return Math.max(16,lines().size()*Minecraft.getInstance().font.lineHeight);}
    ...
    private void draw(GuiGraphicsExtractor g,int x,int y,List<String> lines,boolean preview){var cfg=PhoenixPetDisplay.config();int textX=x;ItemStack icon=PhoenixPetDisplay.icon();if(preview&&icon.isEmpty())icon=new ItemStack(Items.PLAYER_HEAD);if(cfg.petDisplayIcon&&!icon.isEmpty()){g.item(icon,x,y);textX+=20;}Font f=Minecraft.getInstance().font;for(int i=0;i<lines.size();i++){int color=i==0?cfg.petDisplayNameColor:i==lines.size()-1&&lines.get(i).contains("%")?cfg.petDisplayProgressColor:cfg.petDisplayInfoColor;g.text(f,lines.get(i),textX,y+i*f.lineHeight,color,true);}}
}
